...
<body>
    <div class="form-container">
        <h2>Teachers Sign-In</h2>
        <form id="adminForm" action="TeacherSignAction.php" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <?php if (mysqli_num_rows($resultDepartment)): ?>
                    <select class="form-select" name="department" id="department">
                        <option value="">Select a department</option>
                        <?php while ($row = mysqli_fetch_assoc($resultDepartment)): ?>
                            <?php
            ...
            <div class="form-group">
                <button type="button" class="toggle-courses-btn" onclick="toggleCourses()">Available Courses</button>
                <div id="coursesSection" class="courses-section">
                    <label for="Course">Teaching Courses:</label>
                    <?php
                    // Fetch courses from the database
                    include ('connection.php');
                    $result = mysqli_query($con, "SELECT * FROM course where status='0'");
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo '<input type="checkbox" id="course' . $row['CourseID'] . '" name="courses[]" value="' . $row['CourseID'] . '">';
                        echo '<label for="course' . $row['CourseID'] . '">' . $row['name'] . '</label><br>';
                    }
                    ?>
                </div>
            </div>
    ...
    <script>
        function toggleCourses() {
            ...
            coursesSection.classList.toggle('active');
        }
    </script>
